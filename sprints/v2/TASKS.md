...
- [x] Task 18: Link, env var, production deploy (P0)
  ...
  - Completed: 2026-09-19 — `vercel link --yes --project mily-english-abhyas --scope mani125slm` (team scope, as Maths) created the project. **Side effects:** connected GitHub repo `manisankar19/mily-english-abhyas` (a push to `main` now deploys to production); wrote `VERCEL_OIDC_TOKEN` under a `# Created by Vercel CLI` comment into `.env.local` (still git-ignored, mode 600); appended `.vercel` and `.env*` to `.gitignore` (committed). `GANESH_ENGLISH` added to **Production** on stdin with no trailing newline, before the first deploy (`vercel env ls`: Secret, Production). `vercel deploy --prod --yes`: deployment `dpl_BppANQtsez2MaHTezb3S5exJnqhL`, READY, production. Vercel build log: `All 7 paper(s) passed validation` then `build ok: dist/index.html 293.0 KB, 7 papers, 400 items, 5 figures, hash ea048e95c2a1…, source check skipped` (same hash prefix as the local build). **Hostname from the project's domains API: `https://mily-english-abhyas.vercel.app`** (plain name, no suffix; public HTTP 200); deployment URL `mily-english-abhyas-jy0rlypws-mani125slm.vercel.app`. File manifest (`/v6/deployments/{id}/files`): 35 files; no `.env*`, no PDFs, no e2e/test/Python scripts; `source/`, `sprints/`, `fixtures/`, `tests/` appear only as empty directory entries (0 children); `validate.js`, `PROJECT-CARD.yml`, `scripts/lib/card.js`, `build.js` present. Token kept in a shell variable, unset after each command. Deployment Protection untouched (production host public).

- [x] Task 19: Live e2e and README URL (P0)
