...
r = run(fx("symbol-plain"))
check(r.returncode == 0 and "symbol" not in r.stdout.lower().replace("symbol-plain", ""), "the plain word \u201csymbol\u201d (a Braille symbol, a road-sign symbol) is NOT a stem flag", f"exit {r.returncode}")
for _n in ("symbolise", "symbolism"):
    r = run(fx(_n))
    check(r.returncode == 1 and "symbol" in r.stdout.lower(), f"the literary sense \u201c{_n}\u201d IS still a stem flag", f"exit {r.returncode}")
r = run(fx("hard-word"))
